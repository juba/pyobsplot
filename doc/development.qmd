...
`pyobsplot` `widget` renderer uses the [anywidget](https://anywidget.dev) Jupyter widget framework. Python source code is in `src/pyobsplot/widget.py`. JavaScript and CSS source code is in `js/` and `css/`.
...
Each time widget JavaScript or CSS code is modified in the `js/` or  `css` directories, it must be bundled with one of the following scripts:
