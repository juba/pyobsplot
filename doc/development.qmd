...
"debug mode" outputs the computed JavaScript plot structure (the one passed to `Plot.plot`) in the browser console. It is only available with the "widget" renderer.
...
- update with `npm install @observablehq/plot` and `npm install d3`
- run `tools/get_plot_methods.js` and update `utils.py`