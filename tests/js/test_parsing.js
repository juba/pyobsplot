...
describe('parse_spec', function () {
    it('should return correct results for base elements', function () {
        ...
        assert.deepStrictEqual(
            parse_spec([1, ['foo', null]]),
            [1, ['foo', null]]
        );
        assert.deepStrictEqual(
            parse_spec({ x: 1, y: { x2: "foo", y2: null } }),
            { x: 1, y: { x2: "foo", y2: null } }
        );
    });
    it('should return correct results for typed elements', function () {
        assert.equal(
            parse_spec({ "pyobsplot-type": "js", "value": "1 + 2" }),
            3
        );
        assert.deepStrictEqual(
            parse_spec({ "pyobsplot-type": "GeoJson", "value": { "pyobsplot-type": "js", "value": 2 } }),
            { "pyobsplot-type": "js", "value": 2 }
        );
        assert.equal(
            parse_spec({ "pyobsplot-type": "datetime", "value": "2023-01-01T14:25:10" }).getTime(),
            (new Date("2023-01-01T14:25:10")).getTime()
        );
        assert.equal(
            parse_spec({ "pyobsplot-type": "function-object", "module": "Math", "method": "random" }),
            Math.random
        );
        assert.equal(
            parse_spec({ "pyobsplot-type": "function", "module": "Math", "method": "min", "args": [1, 2, 3] }),
            Math.min(1, 2, 3)
        );
    });
